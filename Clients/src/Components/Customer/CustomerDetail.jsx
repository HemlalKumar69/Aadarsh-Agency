...
function CustomerDetail() {
  ...
  const addCustomerFirstInputRef = useRef(null);
  const tableContainerRef = useRef(null); // 👈 for scrolling only table data
  ...
  useEffect(() => {
    if (activeTab === "details" && searchInputRef.current) {
      searchInputRef.current.focus();
    } else if (activeTab === "add" && addCustomerFirstInputRef.current) {
      addCustomerFirstInputRef.current.focus();
    }
  }, [activeTab]);

  // ✅ Keyboard navigation between tabs + scroll only table data
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      // 👇 Scroll only table content (not whole page)
      else if (["ArrowDown", "ArrowUp"].includes(e.key)) {
        if (activeTab === "details" && tableContainerRef.current) {
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="d-flex gap-2">
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip id={`tooltip-edit-${row._id}`}>Edit</Tooltip>}
          >
            <Button
              variant="warning"
              className="d-flex align-items-center justify-content-center"
          ...
          <OverlayTrigger
            placement="top"
            overlay={<Tooltip id={`tooltip-delete-${row._id}`}>Delete</Tooltip>}
          >
            <Button
              variant="danger"
              className="d-flex align-items-center justify-content-center"
          ...
          >
            ...
            >
              <AddCustomer
                ...
                setEditingCustomer={setEditingCustomer}
                setActiveTab={setActiveTab}
                firstInputRef={addCustomerFirstInputRef}
              />
            </Tab>
          </Tabs>
        </Card.Body>
