...
import React, { useState, useEffect, useRef } from "react";
import axios from "../../../Config/axios";
import { Tab, Tabs, Modal, Button, Table, Form } from "react-bootstrap";
import "bootstrap/dist/css/bootstrap.min.css";
import AddTask from "./AddCompany";
import toast from "react-hot-toast";
import Loader from "../../Loader";
...
const CompanyDetail = () => {
  ...
  // ✅ Auto focus search input when in View tab
  useEffect(() => {
    if (activeTab === "view" && searchRef.current) {
  ...
  useEffect(() => {
    if (!searchTerm) setFilteredCompanies(companies);
    else {
      const lower = searchTerm.toLowerCase();
      setFilteredCompanies(
        companies.filter((c) => (c.name || "").toLowerCase().includes(lower))
  ...
  // ✅ Keyboard Navigation (Tab control + table scroll)
  useEffect(() => {
    const handleKeyNavigation = (e) => {
      ...
      // Scroll table with up/down arrow only in "view"
      if (activeTab === "view") {
        const container = tableContainerRef.current;
        if (container && (e.key === "ArrowDown" || e.key === "ArrowUp")) {
          e.preventDefault();
        ...
        <div className="col-md-12">
          <div className="card card-primary card-outline">
            ...
            <div className="card-body">
              {activeTab === "view" && (
                <div>
                  {/* ✅ Sticky search bar + pagination control */}
                  <div
                    style={{
                      position: "sticky",
                      top: 0,
