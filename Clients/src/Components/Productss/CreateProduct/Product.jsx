...
const Product = ({ onSuccess, onCancel, productToEdit }) => {
  ...
  const handleChange = (e) => {
    ...
    // Fields where we want to limit decimals
    const decimalFields = [
      "mrp",
      "salesRate",
      "purchaseRate",
    ...
    if (type === "number" && value !== " ") {
      ...
      if (value !== " ")
        if (name === "availableQty" && parseFloat(value) < 0) {
          // Limit availableQty to non-negative
          processedValue = " ";
        }

      // Limit to 2 decimals
      if (decimalFields.includes(name)) {
        const regex = /^\d*\.?\d{0,2}$/; // allow max 2 decimals
        if (!regex.test(value)) {
    ...
    try {
      ...
      if (photo) {
        ...
        const dataToSubmit = {
          ...
          primaryPrice: formData.primaryPrice === 0 ? " ": formData.primaryPrice,
          secondaryPrice: formData.secondaryPrice === 0 ? " ": formData.secondaryPrice,
          lastUpdated: new Date(),
        };

        if (formData._id) {
          await axios.put(`/product/${formData._id}`, dataToSubmit);
          toast.success("Product updated successfully!");
        } else {
          await axios.post("/product", dataToSubmit);
