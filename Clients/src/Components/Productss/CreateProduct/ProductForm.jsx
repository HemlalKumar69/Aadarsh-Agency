...
const ProductForm = ({ onSuccess, productToEdit }) => {
  ...
  // ✅ File choose karte hi "Create Product" button par focus
  setTimeout(() => {
    document.getElementById("createProductBtn")?.focus();
  }, 300);
};
  ...
  const openSelectDropdown = (selectEl) => {
    if (selectEl) {
      const event = new KeyboardEvent("mousedown", { bubbles: true });
  ...
  if (e.key === "Enter" && input.id === "createProductBtn") {
  e.preventDefault();
  handleSubmit(e);
}
  ...
  const next = () => {
    ...
    if (nextIndex < totalFields) {
      ...
      // 🟢 Agar next input ka name "unit" hai, to dropdown open karo
      if (nextInput?.name === "unit") {
        setTimeout(() => {
          const event = new MouseEvent("dropdown", { bubbles: true });
  ...
  if (e.key === "Enter") {
    e.preventDefault();
    if (input.type === "file") {
      imageInputRef.current?.click(); // file input ke liye
    } else {
      next();
    }
  }

  // Escape: previous field
  if (e.key === "Escape") {
    e.preventDefault();
    prev();
  }
  ...
  // 🔹 Arrow keys
  if (e.key === "ArrowDown") {
    // Agar current input SELECT hai, to native behavior allow karo
  ...
  // Left / Right arrows for text inputs
  if (e.key === "ArrowLeft") {
    try {
...
//   if (input.type === "file") {
//     imageInputRef.current?.click(); // ✅ Enter dabate hi file dialog khulega
//   } else {
//     next();
//   }
// }
      ...
      <div className="card shadow border-0">
        <div className="card-body">
          <form onSubmit={handleSubmit}>
            <div className="row">
              ...
              <div className="col-md-6 mb-3">
                ...
                <select
                  ...
                  onFocus={(e) => openSelectDropdown(e.target)}
              ...
              <div className="col-md-6 mb-3">
                ...
                <input
                  ref={(el) => {
                    ...
                    imageInputRef.current = el; // ✅ image input ref store
                  }}
            ...
            <div className="">
              <button id="createProductBtn" type="submit" className="btn btn-primary">
